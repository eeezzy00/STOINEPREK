...
func log_attack(direction: String, distance: float, hit: bool) -> void:
	_write("ATTACK dir=%s distance=%.1f result=%s" % [direction, distance, "HIT" if hit else "MISS"])
	if hit:
		attacks_landed += 1
...
func _append_summary_csv(won: bool) -> void:
	...
	if f == null:
		return
	if not file_existed:
		f.store_line("run,won,duration,kills,parry_attempts,parry_successes,parry_perfects,parry_success_pct,avg_hit_distance,avg_miss_distance,dashes")
	f.seek_end()
	f.store_line("%d,%s,%.2f,%d,%d,%d,%d,%.1f,%.1f,%.1f,%d" % [
		run_count, won, run_elapsed(), kills,
		parry_attempts, parry_successes, parry_perfects, parry_success_pct(),
		average(hit_distances), average(miss_distances), dash_count,
	])
	f.close()
